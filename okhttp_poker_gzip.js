...
var Cls_okio_GzipSource="okio.GzipSource"
...
function printerResponse(response, logString) {
    ...
    try {
        var defChatset = CharsetWapper.forName("UTF-8")
        var EncodingKey=JavaStringWapper.$new("content-encoding")
        var EncodingGzip=JavaStringWapper.$new("gzip")
        ...
        var encoding=JavaStringWapper.$new("")
        for (var i = 0; i < respHeaderSize; i++) {
            ...
            if(getHeaderName(respHeadersList, i).equals(JavaStringWapper.$new("content-encoding"))){
                encoding=getHeaderValue(respHeadersList,i)
            }
        }
        //Body
        logString.append("| ").append("\n")
        ...
        if (nobody) {
        ...
        }else if(encoding.equals(EncodingGzip)) {
            var charset = defChatset
            ...
            if (!isPlaintext(rspByteString)) {
                logString.append("|" + "<-- END HTTP RESPONSE(binary body omitted)").append("\n");
            }
            if (contentLength != 0) {
                try {
            ...
            logString.append("|" + "<-- END HTTP RESPONSE(encoded body omitted)").append("\n")
        }
    } catch (error) {
...
function bodyEncoded(namesAndValuesList) {
    ...
    if(contentEncoding==''||contentEncoding==undefined){
        contentEncoding = getByHeader(namesAndValuesList, "content-encoding")
    }
...
function hold() {
    Java.perform(function () {
        ...
        OkioGzipSource=Java.use(Cls_okio_GzipSource)
